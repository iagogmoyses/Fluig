...
function setSelectedZoomItem(selectedItem) {
	...
	if (tipo == "centro_custo" ) {
        ...
        $("#cod_centro_custo").val(selectedItem.codCcusto);
        $("#dtIni").val(selectedItem.dtIni);
        $("#dtFim").val(selectedItem.dtFim);
        $("#cod_altera").val(selectedItem.codCcusto);
        $("#tit_altera").val(selectedItem.descricao);
        $("#dtIni_altera").val(selectedItem.dtIni);
        $("#dtFim_altera").val(selectedItem.dtFim);
	}

    if (tipo == "centro_custo_bloq" ) {
		$("#centro_custo_bloq").val(selectedItem.campoPesquisa);
		$("#cod_centro_custo_bloq").val(selectedItem.codCcusto);
        $("#dtIni_bloq").val(selectedItem.dtIni);
        $("#dtFim_bloq").val(selectedItem.dtFim);
        console.log("Centro custo = " + selectedItem.campoPesquisa);
	}
}

function zooms(ev){

	if ( ev == 'centro_custo' && fnBtnLiberado("btncentro_custo") ) {
		if(getWKNumState() == 0 || getWKNumState() == 4 || getWKNumState() == 14){
			var filtro = "campoPesquisa,";
			console.log("Filtro:" + filtro);
			modalzoom.open("ems_consulta_centro_custos_2", "codCcusto,Código,descricao,Descrição,dtIni,Data Início,dtFim,Data Final", "centro_custo", "Pesquisa Centro de Custo", filtro , 'centro_custo' , "" , "", "campoPesquisa" );
		}
    }

    if ( ev == 'centro_custo_bloq' && fnBtnLiberado("btncentro_custo_bloq") ) {
		if(getWKNumState() == 0 || getWKNumState() == 4 || getWKNumState() == 14){
			var filtro = "campoPesquisa,";
			console.log("Filtro:" + filtro);
			modalzoom.open("ems_consulta_centro_custos_2", "codCcusto,Código,descricao,Descrição,dtIni,Data Início,dtFim,Data Final", "centro_custo_bloq", "Pesquisa Centro de Custo", filtro , 'centro_custo_bloq' , "" , "", "campoPesquisa" );
		}
    }
