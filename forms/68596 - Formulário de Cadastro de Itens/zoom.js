...
function setSelectedZoomItem(selectedItem) {
	...
	if (tipo == "itemAltera" ) {
		$("#itemAltera"      ).val("");
		$("#itemAltera").val(selectedItem.campoPesquisa);
		$("#familia_antiga").val(selectedItem.familia);
        $("#unidade_antiga").val(selectedItem.un);
		$("#status_antigo").val(selectedItem.codObsoleto);
        $("#descricao_antiga").val(selectedItem.descItem);
	}

	if (tipo == "status_novo" ) {
		$("#status_novo"      ).val("");
		$("#status_novo").val(selectedItem.status);
	}
}

function zooms(ev){

	if ( ev == 'itemAltera' && fnBtnLiberado("btnitemAltera") ) {
		if(getWKNumState() == 0 || getWKNumState() == 1){
			var filtro = "campoPesquisa,";
			console.log("Filtro:" + filtro);
			modalzoom.open("ems_consulta_item_cadastro", "codItem,Código,descItem,Descrição,un,Unidade de Medida", "itemAltera", "Pesquisa Itens", filtro , 'itemAltera' , "" , "", "campoPesquisa" );
		}
    }

	if (ev == "status_novo" && fnBtnLiberado("btnstatus_novo")){
		if(getWKNumState() == 0 || getWKNumState() == 1){
			
			var filtro  = "status,1- Ativo|2- Obsoleto Ordens Automáticas|3- Obsoleto Todas as Ordens|4- Totalmente Obsoleto";
			modalzoom.open("dsGenerico", "status,Status", "status", "Pesquisa Status Item", filtro , "status_novo", "" , "", "" );
		}
	}
