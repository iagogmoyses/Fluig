...
function setSelectedZoomItem(selectedItem) {
	...
	if (tipo.substring(0,12) == "centro_custo" ) {
	    var idx = tipo.substring(tipo.lastIndexOf("___"), tipo.length);
		$("#centro_custo" + idx      ).val("");
		$("#centro_custo" + idx).val(selectedItem.campoPesquisa);
        $("#cod_centro_custo" + idx).val(selectedItem.codCcusto);
	}

    if (tipo.substring(0,7) == "usuario" ) {
		var idx = tipo.substring(tipo.lastIndexOf("___"), tipo.length);
		$("#usuario" + idx).val(selectedItem.campoPesquisa);
		$("#cod_usuario" + idx).val(selectedItem.codUsuario);
	}
...
function zooms(ev, idObj){
	...
	if ( ev == 'centro_custo' && fnBtnLiberado("btncentro_custo") ) {
		if(getWKNumState() == 0 || getWKNumState() == 1 || getWKNumState() == 4){
			...
			modalzoom.open("ems_consulta_centro_custos_2", "codCcusto,Código,descricao,Descrição,dtIni,Data Início,dtFim,Data Final", "centro_custo", "Pesquisa Centro de Custo", filtro , 'centro_custo_' + idObj , "" , "", "campoPesquisa" );
		}
    }
